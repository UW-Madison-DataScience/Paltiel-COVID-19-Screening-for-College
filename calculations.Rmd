...
```{r}
num.exogenous.shocks <- as.numeric(exogenous.shocks)
cycles.per.day <- 3
...
df <- 
  ...
  mutate(New = lag(Asympt,1,NA)*beta*lag(Susceptible,1,NA)/(lag(Susceptible,1,NA)+lag(Exposed,1,NA)+lag(Asympt,1,NA)),
         New = ifelse(is.na(New),0,New),
         Cumulative = cumsum(New),
         Per.Cumulative = Cumulative/initial.susceptible)
...
sum.stat <- 
  ...
  summarize(`Total Persons Tested in 80 days` = sum(`Persons Tested`, na.rm = TRUE),
            ...
            `Total Infections` = last(Cumulative))
